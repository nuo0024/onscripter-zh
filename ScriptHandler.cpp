...
int ScriptHandler::getStringFromInteger( char *buffer, int no, int num_column, bool is_zero_inserted )
{
    ...
    if (is_zero_inserted){
        for (i=0 ; i<num_space ; i++){
            buffer[c++] = LOC_TWOBYTE_SYMBOL('0')[0];
            buffer[c++] = LOC_TWOBYTE_SYMBOL('0')[1];
        }
    }
    else{
        for (i=0 ; i<num_space ; i++){
            buffer[c++] = LOC_TWOBYTE_SYMBOL(' ')[0];
            buffer[c++] = LOC_TWOBYTE_SYMBOL(' ')[1];
        }
    }
    if (num_minus == 1){
        buffer[c++] = LOC_TWOBYTE_SYMBOL('-')[0];
    ...
    c = (num_column-1)*2;
    const char *num_str = LOC_TWOBYTE_NUM_STR;
